  ...
  describe("Contract Functionality", async function () {
    ...
    it("initializing the contract sets the correct on-chain states", async function () {
      ...
      it("Claiming early access licenses claims new early access licenses, even in 2 times and sets the correct on-chain state", async function () {
          ...
          //claim 1 -- should succeed
          const data = ethers.AbiCoder.defaultAbiCoder().encode(["uint256","uint256"],[0,2]);
          await expect(users[10].PlayFiLicenseSale.claimLicenseEarlyAccess(1,data,proof,{value: ethers.parseEther("0.01")})).to.emit(contracts.PlayFiLicenseSale,"EarlyAccessLicensesClaimed").withArgs(users[10].address,1);
          //claim 2 -- should succeed
          const data2 = ethers.AbiCoder.defaultAbiCoder().encode(["uint256","uint256"],[0,2]);
          await expect(users[10].PlayFiLicenseSale.claimLicenseEarlyAccess(1,data2,proof,{value: ethers.parseEther("0.01")})).to.emit(contracts.PlayFiLicenseSale,"EarlyAccessLicensesClaimed").withArgs(users[10].address,1);
          //claim 3 -- should fail
          const data3 = ethers.AbiCoder.defaultAbiCoder().encode(["uint256","uint256"],[0,2]);
      ...
      it("Claiming partner licenses claims new partner licenses, even in 2 times and sets the correct on-chain state", async function () {
          ...
          //claim 1 -- should succeed
          const data = ethers.AbiCoder.defaultAbiCoder().encode(["uint256","uint256"],[0,2]);
          await expect(users[10].PlayFiLicenseSale.claimLicensePartner(1,data,proof,{value: ethers.parseEther("0.02")})).to.emit(contracts.PlayFiLicenseSale,"PartnerLicensesClaimed").withArgs(users[10].address,1);
          //claim 2 -- should succeed
          const data2 = ethers.AbiCoder.defaultAbiCoder().encode(["uint256","uint256"],[0,2]);
          await expect(users[10].PlayFiLicenseSale.claimLicensePartner(1,data2,proof,{value: ethers.parseEther("0.02")})).to.emit(contracts.PlayFiLicenseSale,"PartnerLicensesClaimed").withArgs(users[10].address,1);
          //claim 3 -- should fail
          const data3 = ethers.AbiCoder.defaultAbiCoder().encode(["uint256","uint256"],[0,2]);
